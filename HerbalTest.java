public class HerbalTest {
    ...
    static double wartosc(Koszyk k, String smak) {
        for (String[] key : Cennik.cennik.keySet()) {
            if (key[1].equals(smak)) {
                for (int i = 0; i < k.zwrocKoszyk().size(); i++) {
                    if (k.zwrocKoszyk().get(i).smak.equals(key[1])) {
                        if (Cennik.cennik.get(key)[0] == 0 || Cennik.cennik.get(key)[0] > k.zwrocKoszyk().get(i).kg) {
                            return k.zwrocKoszyk().get(i).kg * Cennik.cennik.get(key)[1];
                        } else {
                            return k.zwrocKoszyk().get(i).kg * Cennik.cennik.get(key)[2];
                        }
                    }
                }
            }
